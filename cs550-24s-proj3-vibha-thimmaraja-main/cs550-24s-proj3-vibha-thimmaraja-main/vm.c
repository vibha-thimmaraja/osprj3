...
{ 
  ...
  { 
      cprintf("Error in CHP: No user process from cpu %d, cr2=0x%x\n", cpuid(), va); //va = rcr2()
      panic("Page_Fault");
  }
  ...
  // page has write perm_S enabled
    if(PTE_W & *pte)
    {
        ...
        panic("Error in CHP: Already writeable");
    }

  if( pte == 0  || !(*pte) || va >= KERNBASE || !PTE_U || ! PTE_P )
  { 
      myproc()->killed = 1;
      cprintf("Error in CHP: Illegal (virtual) addr on cpu %d address 0x%x, killing proc %s id (pid) %d\n", cpuid(), va, myproc()->name, myproc()->pid);

      return;
  }

    uint pa = PTE_ADDR(*pte);                     //get physical addr
    uint refC = get_reference_count(pa);                // get ref. count of curr. page

    if(refC < 1)
    {
        panic("Error in CHP: Incorrect Reference Count");
    }
    ...
    else                       //refC > 1
    {
        ...
        cprintf("Error in CHP: Out of memory, kill proc %s with pid %d\n", myproc()->name, myproc()->pid);          
        return;
